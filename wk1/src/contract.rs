...
const CONTRACT_NAME: &str = "crates.io:wk1";
...
pub mod execute {
    ...
    // decrement by a num
    pub fn decrement_by(deps: DepsMut, num: i32) -> Result<Response, ContractError> {
        STATE.update(deps.storage, |mut state| -> Result<_, ContractError> {
            state.count -= num;
            Ok(state)
        })?;
    ...
    // increment by a num
    pub fn increment_by(deps: DepsMut, num: i32) -> Result<Response, ContractError> {
        STATE.update(deps.storage, |mut state| -> Result<_, ContractError> {
            state.count += num;
            Ok(state)
        })?;
